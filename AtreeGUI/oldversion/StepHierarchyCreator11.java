...
public class StepHierarchyCreator11 {
...
// hierarchyCreator method should be called from outside of class to add, delete,connect steps.also to create loop
    public void hierarchyCreator()
    {
        ...
        deleteStep.setOnMouseClicked(new EventHandler<MouseEvent>()
        {
            ...
            public void handle(MouseEvent t) {
            	...
            	VBox deleteStepDetailsContainerVBox = new VBox();
            	...
            	deleteStepDetailsContainerVBox.setAlignment(Pos.CENTER);
            	okCancleBtnCntHBox.setPadding(new Insets(0, 0, 10, 0));
            	deleteMessageHBox.setPadding(new Insets(10, 10, 10, 10));
            	deleteStepDetailsContainerVBox.getChildren().addAll(deleteMessageHBox,okCancleBtnCntHBox);
                Scene scene = new Scene(deleteStepDetailsContainerVBox,400,125);
            	...
            	String selectNodeIdArray[] = selectNodeId.split("_");
             	StepProperties selectedStepProp = stepIdAndItsProperties.get(selectNodeIdArray[0]);
              ...
              @Override
              public void handle(MouseEvent t) {       
            	Pane container = selectedStepProp.getStepPane();
            	double width =  container.getWidth();
            	Pane parentContainer = selectedStepProp.getParentStep();
            	//Pane parentContainer = (Pane) childParent.get(selectNodeIdArray[0]);             	
            	String parentNodeId = parentContainer.getId(); 
            	int selectNodeIndex = Integer.parseInt(selectNodeIdArray[1]);
            	String parentNodeIdArray[] = parentNodeId.split("_");
            	StepProperties parentStepProp = stepIdAndItsProperties.get(parentNodeIdArray[0]);
            	//HBox parentHBox = (HBox) stepAndSiblingContainer.get(parentNodeIdArray[0]);
            	HBox parentHBox = parentStepProp.getSiblingHBoxOfStep();
                ...
                String ployLineName = parentNodeIdArray[0] + "_" + selectNodeIdArray[0];
                String pathName = parentNodeIdArray[0] + "_" + selectNodeIdArray[0];
                ...
                Path stepPath = (Path) stepsConnectPath.get(pathName);
                ...
                parentContainer.getChildren().remove(stepPath);
                stepsConnectPath.remove(stepPath);
                //stepAndSiblingContainer.remove(selectNodeIdArray[0]);
                deleteSubStepsFromRowPropAndParentProp(selectNodeIdArray[0]);
                adjustStepNavigationForDelete(selectNodeIdArray[0],width);
    ...
    public void deleteSubStepsFromRowPropAndParentProp(String selectNodeStep)
    {
    	...
    	Pane parentPane = selectedStep.getParentStep();
    	String[] paneArray = parentPane.getId().split("_");
    	String parentId = paneArray[0];
    	//System.out.println(parentId);
    	String[] arr1 = paneArray[0].split("-");
    	//System.out.println(arr1[1]);
    	String[] arr2 = arr1[1].split("\\.");
    	int currentRow = 0;
    	if(!paneArray[0].equals("START-0"))
    	{
    	 currentRow = Integer.parseInt(arr2[0]) + 1;
    	}
    	else
    	{
    	 currentRow = 1;
    	...
    	StepProperties parentProp = stepIdAndItsProperties.get(parentId);
    	List<Pane> childList = parentProp.getStepChildrenList();
    	childList.remove(selectedStep.getStepPane());
    	parentProp.setStepChildrenList(childList);
    	RowProperties curRowProp = rowAndItsProperties.get(currentRow);
    	List<String> rowMem = curRowProp.getRowMember();
    	//System.out.println(rowMem);
    	paneArray = selectedStep.getStepPane().getId().split("_");
    	rowMem.remove(paneArray[0]);
    	//System.out.println(rowMem);
    	if(selectedStep.getStepChildrenCount() > 0)
    	{
        	curRowProp = rowAndItsProperties.get(currentRow + 1);
        	rowMem = curRowProp.getRowMember();
        	//System.out.println(rowMem);
       		List<Pane> childrenPane = selectedStep.getStepChildrenList();
       		for(Pane childPane : childrenPane)
       		{
            	String[] paneRemoveArray = childPane.getId().split("_");
            	String childId = paneRemoveArray[0]; 
            	rowMem.remove(childId);
            	deleteSubStepsFromRowProp(childId,currentRow + 1);
       		}  
       		//System.out.println(rowMem);
    	}  
    ...
    public void deleteSubStepsFromRowProp(String child, int rowNum)
    {
    	StepProperties selectedStep = stepIdAndItsProperties.get(child);
    	...
    	//stepIdAndItsProperties.remove(child);
    }
    ...
    public void adjustStepNavigationForDelete(String stepId, double width)
    {
	  ...
	  StepProperties deletedStepProp = stepIdAndItsProperties.get(stepId2);
	  if(sourceStepAndListOfPath.containsKey(stepId2))
	  {
      List<Path> allSrcPath = sourceStepAndListOfPath.get(stepId2);
      for(Path curPath : allSrcPath)
       {
    	  String pathName = curPath.getId();
    	  stepsConnectPath.remove(pathName);
    	  vb.getChildren().remove(curPath);
    	  sourceStepAndListOfPath.remove(stepId2);
       }	
	  }
	  if(destinationStepAndListOfPath.containsKey(stepId2))
	  {
      List<Path> allDestPath = destinationStepAndListOfPath.get(stepId2);
      for(Path curPath : allDestPath)
       {
    	  String pathName = curPath.getId();
    	  stepsConnectPath.remove(pathName);
    	  vb.getChildren().remove(curPath);
    	  destinationStepAndListOfPath.remove(stepId2);
       }
	  }	  
	  ...
	  {
  	    deleteSubStepsAndItsPaths(childrenlist,stepId2);
	  }
      stepIdAndItsProperties.remove(stepId2);
      selectedStepNode = null;
   	}
	...
	public String twoStepConnector(String sourceStep, String destinationStep )
	{
        ...
        int VerticalIndex = 0, sourceHorizontalIndex = 0, destinationHorizontalIndex = 0;
        ...
        String baseRowAndIndexString = sourceRowNum + "_" + sourceHorizontalFactorTemp;
        ...
        {   
         if(sourcePosXTemp != sourcePosX)
          {
        	sourceRowAndIndexString = sourceRowNum + "_" + sourceHorizontalFactorTemp;
