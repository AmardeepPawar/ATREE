...
public class StepHierarchyCreator12 {
...
// hierarchyCreator method should be called from outside of class to add, delete,connect steps.also to create loop
    ...
    public void deleteSubStepsFromRowPropAndParentProp(String selectNodeStep)
    {
    	...
    	String[] parentStepArraySepByUnderScore = parentPane.getId().split("_");
    	String parentId = parentStepArraySepByUnderScore[0];
    	//System.out.println(parentId);
    	String[] parentStepArraySepByDash = parentStepArraySepByUnderScore[0].split("-");
    	//System.out.println(arr1[1]);
    	String[] parentStepArraySepByDot = parentStepArraySepByDash[1].split("\\.");
    	int deleteStepsRow = 0;
    	if(!parentStepArraySepByUnderScore[0].equals("START-0"))
    	{
    		deleteStepsRow = Integer.parseInt(parentStepArraySepByDot[0]) + 1;
    	}
    	else
    	{
    		deleteStepsRow = 1;
    	...
    	//System.out.println(childList.contains(selectedStep.getStepPane()));
    	...
    	String[] delStepArraySepByUnderScore = selectedStep.getStepPane().getId().split("_");
    	deleteStepsRowMems.remove(delStepArraySepByUnderScore[0]);
    	//System.out.println(rowMem);
    	if(selectedStep.getStepChildrenCount() > 0)
    	{
    		deleteStepsRowProp = rowAndItsProperties.get(deleteStepsRow + 1);
        	deleteStepsRowMems = deleteStepsRowProp.getRowMember();
        	//System.out.println(rowMem);
       		List<Pane> childrenPane = selectedStep.getStepChildrenList();
       		for(Pane childPane : childrenPane)
       		{
            	String[] paneRemoveArray = childPane.getId().split("_");
            	String childId = paneRemoveArray[0]; 
            	deleteStepsRowMems.remove(childId);
